...
public class StringFast
{
    ...
    public byte[] GetBytes(Encoder encoder)
    {
        byte[] bytes = new byte[m_bufferPos];
        return GetBytes(encoder, bytes);
    }

    public byte[] GetBytes(Encoder encoder, byte[] bytes)
    {
        encoder.GetBytes(m_buffer, 0, m_bufferPos, bytes, 0, true);
        return bytes;
    }
    ...
    ///<summary>Append a StringFast without memory allocation</summary>
    public StringFast Append(StringFast value)
    {
        int n = value.m_bufferPos;
        ReallocateIFN(n);
        for (int i = 0; i < n; i++)
            m_buffer[m_bufferPos + i] = value.m_buffer[i];
        m_bufferPos += value.m_bufferPos;
        return this;
    }
    ...
    public StringFast Append(double value)
    {
        m_isStringGenerated = false;
        ReallocateIFN(64); // Check we have enough buffer allocated to handle any double number
        ...
        // Get the 16 meaningful digits as a long
        int nbDecimals = 0;
